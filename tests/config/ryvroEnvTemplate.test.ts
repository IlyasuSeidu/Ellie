...
describe('Ryvro environment template', () => {
  ...
  it('pins dynamic Expo config to Ryvro identity even without inherited static config', () => {
    ...
    process.env.EXPO_ANDROID_GOOGLE_SERVICES_FILE = './missing-google-services.json';
    ...
    try {
      ...
      expect(dynamicConfig.android?.googleServicesFile).toBe('./missing-google-services.json');
  ...
  it('keeps final EAS submit readiness behind an owner evidence guard', () => {
    ...
    expect(script).toContain('scanEvidenceForSecrets');
    expect(script).toContain('BEGIN (?:[A-Z ]+)?PRIVATE KEY');
    expect(script).toContain('"private_key"');
    expect(script).toContain('"private_key_id"');
    expect(script).toContain('AIza');
    expect(script).toContain('appl');
    expect(script).toContain('goog');
    expect(script).toContain('eas_');
  ...
  it('keeps the README release status aligned with current Ryvro launch readiness', () => {
    ...
    expect(readme).toContain('113 Jest suites / 1,814 tests / 4 snapshots');
    ...
    expect(readme).toContain('GitHub Actions CI run `27530645253`');
    expect(readme).toContain('commit `60a0545`');
    expect(readme).toContain('Recent verified E2E workflow gate');
    expect(readme).toContain('GitHub Actions run `27530759317`');
    ...
    expect(readme).toContain('App Store and Google Play subscription-product completion');
    expect(readme).toContain('TestFlight build `4` iPhone smoke test');
    expect(readme).toContain('[docs/RYVRO_RELEASE_READINESS_REPORT.md]');
    expect(readme).toContain('Works at remote work locations');
    expect(readme).toContain('Testing infrastructure (1,814 tests in the latest release check)');
    ...
    expect(readme).toContain('Jest (1,814 tests in the latest release check)');
    expect(readme).toContain('Current Status (as of 2026-06-09 release check)');
    expect(readme).toContain('Total Tests**: 1,814 passing (113 Jest suites, 4 snapshots)');
  ...
  it('keeps the testing strategy aligned with the current Ryvro launch gates', () => {
    const testingStrategy = fs.readFileSync(
      path.join(process.cwd(), 'docs/TESTING_STRATEGY.md'),
      'utf8'
    );

    expect(testingStrategy).toContain('Current release evidence on June 14, 2026');
    expect(testingStrategy).toContain('113 Jest suites / 1,814 tests / 4 snapshots');
    expect(testingStrategy).toContain('The PR CI pipeline must pass');
    expect(testingStrategy).toContain('the dedicated Release Check job');
    expect(testingStrategy).toContain('the Ryvro native scaffold preflight');
    expect(testingStrategy).toContain('the store readiness preflight');
    ...
    expect(testingStrategy).toContain('dashboard-scroll-view');
    expect(testingStrategy).toContain('shift-status-universal-icon');
    expect(testingStrategy).toContain('Run npm run release:check');
    expect(testingStrategy).not.toContain('LoginScreen');
    expect(testingStrategy).not.toContain('home-screen');
    expect(testingStrategy).not.toContain('Unit Tests (70% of tests)');
    expect(testingStrategy).not.toContain('End-to-End Tests (10% of tests)');
    expect(testingStrategy).not.toContain('Run Detox tests on iOS');
    expect(testingStrategy).not.toContain('Run Detox tests on Android');
  });

  it('keeps the architecture guide aligned with current Ryvro screen names', () => {
    const architecture = fs.readFileSync(path.join(process.cwd(), 'docs/ARCHITECTURE.md'), 'utf8');

    expect(architecture).toContain('Universal Shift Builder Architecture');
    expect(architecture).toContain('MainDashboardScreen.tsx');
    expect(architecture).toContain('UniversalShiftBuilderScreen.tsx');
    expect(architecture).toContain('ProfileScreen.tsx');
    expect(architecture).toContain('SignInScreen.tsx');
    expect(architecture).toContain('function SignInScreen()');
    expect(architecture).toContain('DashboardTab');
    expect(architecture).toContain('BuilderTab');
    expect(architecture).toContain('CenterVoiceAction');
    expect(architecture).toContain("import('@/screens/main/ProfileScreen')");
    expect(architecture).not.toContain('LoginScreen');
    expect(architecture).not.toContain('HomeScreen');
    expect(architecture).not.toContain('DetailsScreen');
    expect(architecture).not.toContain('SettingsTab');
    expect(architecture).not.toContain('PreferencesScreen');
    expect(architecture).not.toContain("import('@/screens/ProfileScreen')");
  });
  ...
  it('keeps the release task checklist on Ryvro and repo-root release paths', () => {
    ...
    expect(releaseTasks).toContain('Last updated: June 15, 2026');
    expect(releaseTasks).toContain(
  ...
  it('keeps the Ryvro release readiness report explicit about evidence and remaining blockers', () => {
    ...
    expect(readinessReport).toContain('113 Jest suites / 1,814 tests');
    ...
    expect(readinessReport).toContain('CI run `27530645253`');
    expect(readinessReport).toContain('commit `60a0545`');
    expect(readinessReport).toContain(
      'Recent verified pushed GitHub Actions check for PR #1 passed on commit `60a0545`'
    );
    expect(readinessReport).toContain('GitHub Actions run `27530759317`');
    expect(readinessReport).toContain(
    ...
    expect(readinessReport).toContain('launch-template copy guard');
  ...
  it('keeps the owner launch runbook sequenced for the remaining account and device work', () => {
    ...
    const revenueCatProductsHandoff = fs.readFileSync(
      path.join(process.cwd(), 'docs/RYVRO_REVENUECAT_PRODUCTS_HANDOFF.md'),
      'utf8'
    );
    ...
    expect(ownerRunbook).toContain('fill-in evidence packet for Ryvro Pro store products');
    ...
    expect(ownerRunbook).toContain('113 Jest suites, 1,814 tests');
    ...
    expect(ownerRunbook).toContain('aligning Ryvro EAS update guidance');
    expect(ownerRunbook).toContain('CI run `27530645253`');
    expect(ownerRunbook).toContain('commit `60a0545`');
    expect(ownerRunbook).toContain('GitHub Actions run `27530759317`');
    ...
    expect(submitBlockerTriage).toContain('service-account private-key JSON fields');
    expect(submitBlockerTriage).toContain('Firebase API keys');
    expect(submitBlockerTriage).toContain('RevenueCat SDK keys');
    expect(submitBlockerTriage).toContain('EAS access tokens');
    expect(submitBlockerTriage).toContain('rotate the exposed key in the owning console');
    ...
    expect(launchEvidenceLog).toContain('HTTP `200` with `status` `draft`');
    expect(launchEvidenceLog).toContain('an 8-item 2 days, 2 nights, 4 off schedule');
    expect(launchEvidenceLog).toContain('schedule name `2-week roster`');
    ...
    expect(launchEvidenceLog).toContain('Key ID `G842654HWJ`');
    expect(launchEvidenceLog).toContain('Do not use `VMFGH4BNTL` or `G842654HWJ`');
    ...
    expect(firebaseOauthBackendHandoff).toContain('HTTP `200` with `status` `draft`');
    ...
    expect(revenueCatProductsHandoff).toContain('# Ryvro RevenueCat And Store Products Handoff');
    expect(revenueCatProductsHandoff).toContain('Entitlement ID: `pro`');
    expect(revenueCatProductsHandoff).toContain('Entitlement display name: `Ryvro Pro`');
    expect(revenueCatProductsHandoff).toContain('Offering ID: `default`');
    expect(revenueCatProductsHandoff).toContain('Monthly product ID: `ryvro_pro_monthly`');
    expect(revenueCatProductsHandoff).toContain('Annual product ID: `ryvro_pro_annual`');
    ...
    expect(revenueCatProductsHandoff).toContain('Do not use `VMFGH4BNTL` or `G842654HWJ`');
    ...
    expect(revenueCatProductsHandoff).toContain('Create offering ID `default`');
    expect(revenueCatProductsHandoff).toContain('REVENUECAT_IOS_KEY=<real appl_ key>');
    expect(revenueCatProductsHandoff).toContain('REVENUECAT_ANDROID_KEY=<real goog_ key>');
    expect(revenueCatProductsHandoff).toContain('npm run release:env:check');
    ...
    expect(googlePlayInternalTestingHandoff).toContain('App name: `Ryvro Shift Planner`');
    expect(googlePlayInternalTestingHandoff).toContain('Package name: `com.ryvro.shiftplanner`');
    expect(googlePlayInternalTestingHandoff).toContain('First release track: Internal testing');
    expect(googlePlayInternalTestingHandoff).toContain(
