...
| 43  | 👤 Add Android app to RevenueCat → copy Android SDK key → paste into `.env` as `REVENUECAT_ANDROID_KEY`                                                                                                           | 🔧 Partial (Android app `Ryvro (Play Store)` exists as `appab0f4b628d`; Android SDK key is copied into local ignored `.env`; Play product validation is still blocked by Google Payments merchant setup) |
| 44  | 👤 RevenueCat → Entitlements → Add entitlement: ID = `pro`, Display name = `Ryvro Pro`                                                                                                                            | 🔧 Partial (entitlement `pro` exists with display name `Ryvro Pro`; keep pending until real App Store and Google Play products validate and sandbox purchase QA passes)                                  |
...
| 48  | 👤 RevenueCat → Offerings → create offering named `default` → add Annual package + Monthly package                                                                                                                | 🔧 Partial (offering `default` exists with monthly and annual packages; store pricing, trial metadata, and production-equivalent purchase loading still need validation)                                 |
