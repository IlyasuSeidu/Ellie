...
| 21  | Build production iOS binary: `eas build --platform ios --profile production`                                                                        | 🔧 Partial (EAS iOS build `c99b0e0a-829c-4ab7-bd93-164586ade68a` uploaded to TestFlight; latest EAS build `782b6dec-1cf1-4cf2-9159-69ef1ab4078a` also finished with version `1.0.0`, build `1`, but should not be submitted as-is because App Store Connect already has build `1`; rebuild still required after real Firebase/OAuth/RevenueCat env evidence and an incremented iOS build number) |
