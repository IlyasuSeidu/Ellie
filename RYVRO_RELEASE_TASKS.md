...
| 16  | 👤 Update `.env`: set `APP_ENV=production`, paste `EAS_PROJECT_ID`, paste Firebase/Google OAuth values, fill `REVENUECAT_IOS_KEY` / `EXPO_PUBLIC_REVENUECAT_IOS_KEY` and `REVENUECAT_ANDROID_KEY` / `EXPO_PUBLIC_REVENUECAT_ANDROID_KEY` from Phase 4, set live HTTPS legal/support URLs, then run `npm run release:env:check` | 👤 Todo |
