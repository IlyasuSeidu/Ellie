...
- Latest verified pushed PR gate: GitHub Actions CI run `27525326188` on commit `817800f` passed Unit Tests, Lint and Type Check, Build Check, and the dedicated Release Check job running `npm run release:check`
- Latest verified E2E workflow gate: GitHub Actions run `27525326580` on commit `817800f` passed the bounded E2E Configuration Check; native simulator/emulator Detox jobs remain available through the manual `run_native` workflow input
