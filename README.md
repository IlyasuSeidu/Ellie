...
- Recent verified pushed PR gate: GitHub Actions CI run `27530645253` on commit `60a0545` passed Unit Tests, Lint and Type Check, Build Check, and the dedicated Release Check job running `npm run release:check`
- Recent verified E2E workflow gate: GitHub Actions run `27530759317` on commit `60a0545` passed the bounded E2E Configuration Check; native simulator/emulator Detox jobs remain available through the manual `run_native` workflow input
