...
- Recent verified pushed PR gate: GitHub Actions CI run `27531970661` on commit `3690178` passed Unit Tests, Lint and Type Check, Build Check, and the dedicated Release Check job running `npm run release:check`
- Recent verified E2E workflow gate: GitHub Actions run `27532102773` on commit `3690178` passed the bounded E2E Configuration Check; native simulator/emulator Detox jobs remain available through the manual `run_native` workflow input
