...
- Firebase Auth email templates are still pending. The earlier metadata-only verification showed default sender and reply-to values before the console edits. Later logged-in Firebase Console work on 2026-06-06 saved Email address verification and Password reset with sender `Ryvro Support` and reply-to `support@getryvro.com`. The action URL attempt for `https://getryvro.com/__/auth/action` did not produce verified persistent evidence; the preview still showed `https://ryvro-shift-planner.firebaseapp.com/__/auth/action`. The repo now includes a Firebase Hosting action handler at `web/launch/auth/action/index.html` and `web/launch/auth/action/handler.js`; use custom action URL `https://getryvro.com/auth/action/` after the `getryvro.com` domain is connected to Firebase Hosting and verified.
