...
- Historical pre-project state: Firebase CLI reauth completed on 2026-06-05 as `seiduilyasu94@gmail.com`; `firebase projects:list` succeeded and returned `ellie-20260220135308`, `gen-lang-client-0387037810`, `iman-app-7e20d`, `qurantree-app`, and `sunnahmind-staging-36c1ac`. No Ryvro project was present at that moment, and the current CLI project remained `ellie-20260220135308`. That state is no longer the launch baseline; it is retained only to explain why deploy commands below pin `--project ryvro-shift-planner`.
...
- Current tracked Firebase source of truth: `.firebaserc` points the default project at `ryvro-shift-planner` and maps hosting target `launch-site` to `ryvro-launch-site`. Keep using `--project ryvro-shift-planner` on deploy and functions commands so local Firebase Tools config-store drift cannot redirect a Ryvro launch deploy to a retired project.
...
- Current generated Android native service-file check on 2026-06-15 confirmed the ignored `android/app/google-services.json` also targets project ID `ryvro-shift-planner`, project number `1002666052675`, storage bucket `ryvro-shift-planner.firebasestorage.app`, package `com.ryvro.shiftplanner`, and Android app ID `1:1002666052675:android:735fd0ef9443ddf76b98f6`. This generated file is ignored by Git and must be replaced from the ignored root `./google-services.json` if a clean prebuild or native build regenerates Android scaffolding.
...
- Keep the generated ignored `android/app/google-services.json` synchronized with the ignored root `./google-services.json` before local Android release builds; `npm run release:native:check` now rejects the generated file if it points at a retired Firebase project.
...
- Firebase Auth email templates are complete for first release through owner-approved first-release fallback evidence. Logged-in Firebase Console work on 2026-06-06 saved Email address verification and Password reset with sender `Ryvro Support` and reply-to `support@getryvro.com`. A metadata-only Identity Toolkit Admin API readback on 2026-06-16 confirmed `authorizedDomains` contains `getryvro.com`, confirmed the Verify email template sender display name is `Ryvro Support`, confirmed the Verify email reply-to is `support@getryvro.com`, confirmed the Password reset template sender display name is `Ryvro Support`, and confirmed the Password reset reply-to is `support@getryvro.com`. The same readback confirmed Firebase still uses callback URI `https://ryvro-shift-planner.firebaseapp.com/__/auth/action`; earlier attempts to set `https://getryvro.com/auth/action/` through the console and API did not persist, and the API patch returned `EMAIL_TEMPLATE_UPDATE_NOT_ALLOWED`. Owner-approved first-release fallback completed on 2026-06-16: keep Firebase's default callback URL for first release, keep `getryvro.com` as an authorized domain and public support/legal domain, use the branded sender and reply-to, and record end-to-end email verification plus password reset smoke results on a production-auth-ready build. Approved by Ilyasu Seidu. Do not record one-time email links, reset links, auth tokens, or tester passwords.
- Required Firebase Auth sender name: `Ryvro Support`
- Required Firebase Auth reply-to email: `support@getryvro.com`
- First-release Firebase Auth callback URL fallback: `https://ryvro-shift-planner.firebaseapp.com/__/auth/action`
