...
- Firebase Auth email templates are still pending. The earlier metadata-only verification showed default sender and reply-to values before the console edits. Later logged-in Firebase Console work on 2026-06-06 saved Email address verification and Password reset with sender `Ryvro Support` and reply-to `support@getryvro.com`. Logged-in Firebase Console recheck on 2026-06-14 confirmed the Email address verification preview still shows sender `Ryvro Support`, reply-to `support@getryvro.com`, and default action URL `https://ryvro-shift-planner.firebaseapp.com/__/auth/action`. The repo now includes a Firebase Hosting action handler at `web/launch/auth/action/index.html` and `web/launch/auth/action/handler.js`, and live `https://getryvro.com/auth/action/` returns HTTP `200` with `Cache-Control: no-store`. After the custom domain went live, Firebase Console accepted the custom action URL dialog without a visible error, but the password reset preview still showed `https://ryvro-shift-planner.firebaseapp.com/__/auth/action`. A metadata-only Identity Toolkit Admin API readback confirmed `callbackUri` is still `https://ryvro-shift-planner.firebaseapp.com/__/auth/action`; an API patch attempt for `https://getryvro.com/auth/action/` returned `EMAIL_TEMPLATE_UPDATE_NOT_ALLOWED`. A 2026-06-14 browser retry set `https://getryvro.com/__/auth/action` in the custom action URL dialog and Firebase returned `Error: An error occurred updating action URL`. Final evidence must be either metadata readback proving `callbackUri` persists as `https://getryvro.com/auth/action/`, Firebase support escalation completing the template update, or an owner-approved first-release fallback that keeps Firebase's default callback, confirms the sender and reply-to remain Ryvro-branded, and records end-to-end email verification plus password reset smoke results on a production-auth-ready build. Do not mark passed from console save toasts alone.
