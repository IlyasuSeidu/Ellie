...
- Remote EAS Android versionCode was checked on 2026-06-14 and still showed `1`, matching the old June 5 AAB. `npx eas-cli@14 build:version:set --platform android --profile production` set it to `2`, and a follow-up version check confirmed Android `versionCode` `2`. A fresh production Android build attempt loaded the production EAS environment and auto-incremented Android versionCode from `2` to `3`, but the local EAS upload stalled at `Uploading to EAS Build (0 / 78.9 MB)`. A parallel `build:list` check still showed only the old June 5 Android build, so no fresh production-auth Android build record was created. Retry the Android production build from a reliable local terminal before Play internal testing.
- A second Android production build retry with current `npx eas-cli@20` loaded the same production EAS environment and auto-incremented Android versionCode from `3` to `4`, but also stalled at `Uploading to EAS Build (0 / 165 MB)` before creating a build record. A parallel `npx eas-cli@20 build:list --platform android --limit 5 --json --non-interactive` still showed only the old June 5 build, and `npm run release:versions:get` then confirmed remote Android `versionCode` `4`. The next Android production build attempt should expect versionCode `5` or higher while `autoIncrement` remains enabled.
- A local Android production build attempt with `npx eas-cli@20 build --platform android --profile production --local --non-interactive` on 2026-06-14 bypassed the remote EAS upload stall and auto-incremented Android `versionCode` from `4` to `5`, but failed before Gradle during the EAS local `npm ci --include=dev` phase with `network read ETIMEDOUT`. A follow-up `npm run release:versions:get` confirmed remote Android `versionCode` is now `5` and iOS build number remains `4`. Retry from a stable network or pre-warm the npm cache before attempting the local Android AAB again; the next auto-incremented Android build should expect versionCode `6` or higher.
