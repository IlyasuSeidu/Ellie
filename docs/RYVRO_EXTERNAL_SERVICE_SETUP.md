...
- Firebase Console accepted the `getryvro.com` custom-domain verification and moved the domain to `Minting certificate`.
...
Record the live `https://getryvro.com/privacy`, `https://getryvro.com/terms`, `https://getryvro.com/support`, `https://getryvro.com/delete-account`, and `https://getryvro.com/auth/action/` checks in `docs/RYVRO_LAUNCH_EVIDENCE_LOG.md`. Record any later certificate-state changes, support mailbox checks, Firebase Auth callback fixes, and store-console URL evidence in the same file.
