...
- Physical XS Max check on 2026-06-14 found device `Thinky iPhone Xs Max` / identifier `67D1E1AF-AE0C-599F-9436-88673C960B9C` available, paired, and unlocked. Device inventory showed Ryvro installed as version `1.0.0`, bundle version `1`, bundle ID `com.ryvro.shiftplanner`; launching `com.ryvro.shiftplanner` succeeded and the process list showed `RyvroShiftPlanner.app/RyvroShiftPlanner`. This is not the production-auth-ready build `4`.
- Build `4` was downloaded from EAS and verified locally on 2026-06-14: `CFBundleIdentifier` `com.ryvro.shiftplanner`, `CFBundleShortVersionString` `1.0.0`, and `CFBundleVersion` `4`. A direct `devicectl device install app` attempt to the XS Max failed with Apple error `ApplicationVerificationFailed`: `Attempted to install a Beta profile without the proper entitlement` and `This app cannot be installed because its integrity could not be verified.` Treat this as proof that build `4` must be installed through TestFlight, not by direct device install.
...
- If the XS Max still shows bundle version `1`, remove or update that install through TestFlight and verify the installed bundle version is `4` before recording QA or store screenshot evidence.
