...
- Spaceship Email forwarding now has `reviewer@getryvro.com` forwarding to `seiduilyasu94@gmail.com`, so the reviewer mailbox address has a destination before store review. On 2026-06-07, the Firebase Auth Users page enabled the Add user flow, and the Add user dialog was opened with `reviewer@getryvro.com` filled as the email, but no password was entered and the final Add user action was not clicked. The Firebase Auth reviewer user and password still must be created only after Email/Password is saved and production auth is ready, and the password must not be stored in Git, docs, screenshots, or chat.
