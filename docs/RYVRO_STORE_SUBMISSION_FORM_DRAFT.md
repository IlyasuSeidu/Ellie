...
- User Content: shift schedules, repeating patterns, known-date setup answers, reminder settings, user profile details, and optional voice question transcripts or audio-derived content.
...
- App Functionality: account access, shift setup, reminders, saved settings, subscription gating, purchase restore, voice assistant, online answers, and offline schedule recovery.
- Analytics: feature usage, setup funnel quality, reliability, and broad launch-persona/template reporting.
- Product Personalization: saved shift pattern, reminder preference, user name, and assistant answers.
...
- User-generated content: shift schedules, shift names, repeating patterns, known-date setup answers, reminder settings, user details, and optional voice question content.
...
- Optional/user-initiated: voice questions, support screenshots/files, and optional profile fields not needed to save a schedule.
...
Ryvro helps shift workers configure a repeating shift once, then ask Ryvro by voice for accurate shift answers. To test: sign in with the reviewer account, complete the simplified setup flow, ask one voice question, confirm the answer card appears, open Settings from the Ask screen, open the Ryvro Pro paywall after the trial answer, and test sandbox purchase or restore. The app is a personal planning tool and does not replace employer rosters, payroll, HR systems, fatigue-management policy, or safety instructions.
