...
- Social handle reservation: owner-approved reserved handle list completed on 2026-06-16 with X `@getryvro`, Instagram `@getryvro`, TikTok `@getryvro`, YouTube `@getryvro`, and LinkedIn `Ryvro`.
...
- Firebase Auth email-template evidence is complete for first release. Metadata readback confirms the Verify email and Password reset templates use sender `Ryvro Support` and reply-to `support@getryvro.com`; owner-approved fallback keeps Firebase's default callback URL for first release after the custom action URL did not persist.
...
- RevenueCat's separate App Store Connect API key upload is complete. On 2026-06-22, App Store Connect key `Ryvro RevenueCat API 3` with Key ID `834JX8UA5R`, Issuer ID `35e6ee90-4048-4a23-8835-1f05427cec0f`, App Manager access, and vendor number `94098500` was uploaded to RevenueCat and validated. The 2026-06-14 `Ryvro RevenueCat API` key with Key ID `VMFGH4BNTL` and second `Ryvro RevenueCat API 2` key with Key ID `G842654HWJ` were revoked after their one-time `.p8` downloads could not be verified locally.
- Configure entitlement ID `pro` with display name `Ryvro Pro`.
- App Store Connect product shells now exist: subscription group `Ryvro Pro` ID `22156776`, monthly product `ryvro_pro_monthly` Apple ID `6780186030`, and annual product `ryvro_pro_annual` Apple ID `6780186069`; both products now show `READY_TO_SUBMIT`.
...
- Finish App Store trial decision, sandbox purchase QA, restore QA, and RevenueCat entitlement activation QA.
...
- Offering ID `default` exists with monthly and annual packages from the current RevenueCat product records, including App Store products `ryvro_pro_monthly` and `ryvro_pro_annual`.
